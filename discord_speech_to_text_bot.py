...
class BufferAudioSink(discord.AudioSink):
    ...
    def write(self, voice_data):
        ...
        frame = np.ndarray(
            ...
            buffer=voice_data.data,
        )
        speaking = np.abs(frame).sum() > 0
        need_flush = (self.buffer_pointer >= self.BUFFER_FRAME_COUNT - 2) or (
            not speaking and self.buffer_pointer > 0.5 * self.BUFFER_FRAME_COUNT
        )  # or (self.speaker is not None and speaker != self.speaker)

        if speaking:
            self.buffer[
                (self.buffer_pointer * self.NUM_SAMPLES) : (
                    (1 + self.buffer_pointer) * self.NUM_SAMPLES
                )
            ] = frame
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--endpoint", default=google.cloud.speech_v1.SpeechClient.SERVICE_ADDRESS
    )
